...
export function ScheduleTable({ data, project, tasks, onTask, onTaskUpdate, onTaskUpdates, onBulkUpdate, onBulkDelete, onAddTask, onAddMilestone, onProjectUpdate, onCaptureBaseline, onReorder, onIndent, onOutdent, history, canUndo, canRedo, onUndo, onRedo, importCsv, exportCsv }: ScheduleTableProps) {
  ...
  return <div className="schedule-sheet panel">
    ...
    <div className="schedule-intelligence">
      <div><button className={analysis.issues.some((issue) => issue.severity === "critical") ? "schedule-health critical" : analysis.issues.length ? "schedule-health warning" : "schedule-health healthy"} onClick={() => setIssuesOpen(true)} aria-expanded={issuesOpen}><AlertTriangle size={13} />{analysis.issues.length ? `${analysis.issues.length} schedule issue${analysis.issues.length === 1 ? "" : "s"}` : "Schedule is healthy"}</button><span>Finish {analysis.finishDate}</span><span>{analysis.criticalTaskIds.size} critical-path task{analysis.criticalTaskIds.size === 1 ? "" : "s"}</span>{tasks.some((task) => task.baselineDueDate) && <span className={analysis.baselineVarianceDays > 0 ? "variance-late" : "variance-good"}>{analysis.baselineVarianceDays > 0 ? `+${analysis.baselineVarianceDays}d vs baseline` : "On baseline"}</span>}</div>
    ...
    {issuesOpen && <div className="schedule-issues-layer"><button className="schedule-issues-backdrop" onClick={() => setIssuesOpen(false)} aria-label="Close schedule issues" /><section className="schedule-issues-dialog" role="dialog" aria-modal="true" aria-labelledby="schedule-issues-title"><header><div><h2 id="schedule-issues-title">Schedule issues</h2><p>Resolve blocked work, missing predecessors, date slips, and circular dependencies.</p></div><button onClick={() => setIssuesOpen(false)} aria-label="Close"><X size={18} /></button></header><div>{analysis.issues.length ? analysis.issues.map((issue) => <button key={issue.id} className={issue.severity} onClick={() => { if (issue.taskId) onTask(issue.taskId); setIssuesOpen(false); }}><AlertTriangle size={14} /><span>{issue.message}</span><ChevronRight size={15} /></button>) : <p className="schedule-healthy-empty">No blocked, late, circular, or missing predecessor issues.</p>}</div></section></div>}
